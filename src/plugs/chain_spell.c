...
static void load_resources(void) {
    ...
    p->cs.shader = LoadShader(0, "./assets/shaders/chain_spell.fs");
    p->cs.timeLoc = GetShaderLocation(p->cs.shader, "u_time");
    p->cs.mouseLoc = GetShaderLocation(p->cs.shader, "u_mouse");
    p->cs.resolutionLoc = GetShaderLocation(p->cs.shader, "u_resolution");
...
static void unload_resources(void) {
    UnloadFont(p->font);
    UnloadShader(p->cs.shader);
    UnloadShader(p->info.shader);
}

void plug_reset(void) {
    p->time = 0.0f;
    p->mousePos = GetMousePosition();
}
...
void plug_update(float dt, float w, float h, bool render) {
    ...
    // Update mouse position only on click
    if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        p->mousePos = GetMousePosition();
    }

    float resolution[2] = { w, h };
    float mouseLoc[2] = { p->mousePos.x, p->mousePos.y};

    // ChainSpell Shader
    BeginShaderMode(p->cs.shader);
        SetShaderValue(p->cs.shader, p->cs.timeLoc, &p->time, SHADER_UNIFORM_FLOAT);
        SetShaderValue(p->cs.shader, p->cs.resolutionLoc, resolution, SHADER_UNIFORM_VEC2);
        SetShaderValue(p->cs.shader, p->cs.mouseLoc, mouseLoc, SHADER_UNIFORM_VEC2);
